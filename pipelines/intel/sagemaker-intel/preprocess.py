...
def generate_train_test_split():
    ...
    dataset_full = list((dataset_extracted).rglob("*/*.jpg"))
    print(f"Length of dataset - {len(dataset_full)} ")
    labels = [x.parent.stem for x in dataset_full]
    
    
    # dataset_full = list((dataset_extracted / "intel-image").glob("*/*.jpg"))
    # labels = [x.parent.stem for x in dataset_full]
    
    print(":: Dataset Class Counts: ", Counter(labels))
    
    # d_train, d_test = train_test_split(dataset_full, stratify=labels)
    d_train = list((dataset_extracted / "seg_train" / "seg_train").rglob("*/*.jpg"))
    d_test = list((dataset_extracted / "seg_test" / "seg_test").rglob("*/*.jpg"))
...
if __name__=="__main__":
    ...
    subprocess.check_call('cp -r /opt/ml/processing/sagemaker-intel_image/dataset/train/* /opt/ml/processing/dataset/train', shell=True)
    subprocess.check_call('cp -r /opt/ml/processing/sagemaker-intel_image/dataset/test/* /opt/ml/processing/dataset/test', shell=True)
